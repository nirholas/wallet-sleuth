...
Braid is a chain analysis tool. Tools like this get used to hold exchanges accountable, to trace
...
## What Braid does and does not do

**It reads public data only.** Every byte Braid uses is already published on a public blockchain and
served by public explorers and RPC endpoints. Braid does not scrape social accounts, does not buy
...
is behind them. Turning that into a name requires information Braid does not have and does not want.
If you bring that information yourself, the responsibility for what you do with the combination is
entirely yours.

**It stores nothing.** Analyses run in memory and are dropped after `BRAID_JOB_RETENTION_MS` (one
...
**Your queries are visible to the providers.** Braid asks public endpoints about specific addresses,
and those endpoints see the requests. If that matters for your use case, run Braid against your own
...
asymmetry is why Braid is built the way it is:
...
Security issues, including ways to make Braid produce confidently wrong results, are welcome and
covered by [SECURITY.md](https://github.com/braid-tools/braid/blob/main/SECURITY.md). A signal that
