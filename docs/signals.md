...
Wallet Sleuth runs 14 signals. Each one produces evidence with a strength between 0 and 1; the
...
Evidence that one key can act for both addresses. This is the strongest class Wallet Sleuth has, because it reads state and signatures rather than inferring intent from behaviour.
