...
Braid is one Node process. It serves the API and the built web client from the same port.
...
docker run -p 8787:8787 -e ETHERSCAN_API_KEY=... braid
```
...
Everything is optional. Braid runs with an empty environment.
...
| `BRAID_CORS_ORIGINS` | none | Comma separated origins allowed to call the API from a browser. Empty means same-origin only. |
| `BRAID_API_KEYS` | none | Comma separated bearer keys. When set, `/v1` requires one. |
| `BRAID_RATE_LIMIT` | `60` | Requests per minute per key or IP. `0` disables. Health checks and the read-only metadata routes are exempt. |
| `BRAID_ANALYZE_RATE_LIMIT` | `10` | Analyses started per minute. The only route that spends upstream requests. |
| `BRAID_MAX_CONCURRENT_JOBS` | `4` | Analyses running at once. Excess submissions queue. |
| `BRAID_JOB_RETENTION_MS` | `3600000` | How long a finished report stays retrievable. |
| `REDIS_URL` | none | Shared provider cache |
| `BRAID_CACHE_TTL_SECONDS` | `900` | Cache lifetime for mutable reads |
...
| `BRAID_LABELS_FILE` | none | Extra service labels, layered on the built-in set |
| `BRAID_HOST_CONCURRENCY` | `6` | In-flight requests per upstream host |
| `BRAID_HOST_MIN_INTERVAL_MS` | `60` | Starting gap between requests to one host |
| `BRAID_HOST_MAX_INTERVAL_MS` | `2000` | Ceiling the adaptive backoff will widen to |
| `BRAID_WEB_ROOT` | auto | Override where the built web client is served from |
...
pay for the same reads. Raising `BRAID_MAX_CONCURRENT_JOBS` past a handful will usually make
everything slower, not faster, because the upstream endpoints are the bottleneck.

**Job storage is in memory.** Reports are held for `BRAID_JOB_RETENTION_MS` in the process that ran
...
**Security.** Set `BRAID_API_KEYS` on anything public, keep `BRAID_CORS_ORIGINS` empty unless a
browser on another origin genuinely needs access, and put it behind TLS. Braid sets a strict
